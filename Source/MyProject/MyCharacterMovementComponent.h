...
public:
	...
	// Called from the saved move, after a correction has been received.
	// When a correction is received from the server, the client state is rollbacked to what the server says, and the all the saved moves are replayed, this may include a movement mode rollback.
...
public:
	...
	//float SavedEndInterpAccTime = 0.0f;
};
